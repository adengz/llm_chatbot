...
from api.main import app, get_db, get_llm, get_disconnect_checker, DBClient, LLMClient
...
class MockLLMStreamer:
    ...
    def __init__(
            self, 
            responses: list[tuple[Literal['thinking', 'tool_call_req', 'tool_call_resp', 'content'], str | BaseModel]], 
            ex: bool = False,
        ):
        self.chunks = []
    ...
    async def stream_response(self, *args, **kwargs) -> AsyncGenerator[AgentStreamChunk, None]:
        ...
        if self.ex:
            yield AgentStreamChunk(type='error', exception='LLM error', status_code=500)
        else:
...
def api_ut_toolkit():
    ...
    yield TestClient(app), mock_db, mock_llm
...
class TestAppEndpoints:
    ...
    def test_list_models(self, api_ut_toolkit):
        client, _, mock_llm = api_ut_toolkit

        models = ['franklin-clinton', 'michael-de-santa', 'trevor-philips']
    ...
    def test_create_message_new_conversation(self, api_ut_toolkit):
        client, mock_db, mock_llm = api_ut_toolkit
