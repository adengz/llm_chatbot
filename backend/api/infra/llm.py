...
from ollama import AsyncClient, ResponseError, web_fetch, web_search
...
class AsyncOllamaClient:
    ...
    async def stream_response(
        ...
        try:
            while tool_calls or not done:
                while tool_calls:
                    ...
                    yield AgentStreamChunk(
                        type="tool_call_req", data=tool_call.function
                    )
                ...
                async for part in await self.client.chat(
                    ...
                    if part.message.tool_calls is not None:
                        ...
                        yield AgentStreamChunk(
                            type="thinking", delta=part.message.thinking
                        )
                    elif part.message.content:
                        yield AgentStreamChunk(
                            type="content", delta=part.message.content
                        )
                    done = part.done
            ...
            yield AgentStreamChunk(
                type="error", exception=exc.error, status_code=exc.status_code
            )
